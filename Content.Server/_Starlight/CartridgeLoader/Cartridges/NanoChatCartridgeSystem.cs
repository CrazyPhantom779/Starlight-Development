...
public sealed class NanoChatCartridgeSystem : EntitySystem
{
    ...
    private void HandleNewChat(Entity<NanoChatCardComponent> card, NanoChatUiMessageEvent msg)
    {
        ...
        {
            name = name.Trim();
            if (name.Length > SharedNanoChatSystem.MaxFullNameLength)
                name = name[..SharedNanoChatSystem.MaxFullNameLength];
        }
        ...
        if (!string.IsNullOrWhiteSpace(jobTitle))
        {
            ...
            if (jobTitle.Length > SharedNanoChatSystem.MaxJobTitleLength)
                jobTitle = jobTitle[..SharedNanoChatSystem.MaxJobTitleLength];
        }
        ...
        _adminLogger.Add(LogType.Action,
            LogImpact.Low,
            $"{ToPrettyString(msg.Actor):player} created new NanoChat conversation with #{msg.RecipientNumber:D4} ({name})");
    ...
    private void HandleDeleteChat(Entity<NanoChatCardComponent> card, NanoChatUiMessageEvent msg)
    {
        ...
        _adminLogger.Add(LogType.Action,
            LogImpact.Low,
            $"{ToPrettyString(msg.Actor):player} deleted NanoChat conversation with #{chatNumber:D4}");
        ...
        NanoChatUiMessageEvent msg)
    {
        ...
        if (!string.IsNullOrWhiteSpace(content))
        {
            ...
            if (content.Length > SharedNanoChatSystem.MaxContentLength)
                content = content[..SharedNanoChatSystem.MaxContentLength];
        }
