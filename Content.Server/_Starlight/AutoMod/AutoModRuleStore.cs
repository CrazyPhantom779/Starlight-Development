...
///
/// This intentionally does not read YAML/prototypes and intentionally does not try
/// to migrate older branch formats. The saved runtime JSON document is the only
/// rule source so admins can create, edit, disable, and delete every rule from the
/// in-game AutoMod UI without opening a PR or touching the codebase.
/// </summary>
internal sealed class AutoModRuleStore
{
    ...
    private static AutoModEditableRule Sanitize(AutoModEditableRule rule)
    {
        ...
        rule.Match.WordSet = null; // No YAML/prototype wordsets. All words are inline, runtime-editable rule data.
        rule.Match.Pattern = string.IsNullOrWhiteSpace(rule.Match.Pattern) ? null : rule.Match.Pattern;
